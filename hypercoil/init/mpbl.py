...
def _mpbl_eval(
    ...
    compressed = (asgt[0] / n_edges_out[0]) @ (
        objective @ (asgt[1] / n_edges_out[1]).swapaxes(-2, -1)
    )
...
def maximum_potential_bipartite_lattice(
    ...
    *,
    ...
    for _ in range(iters):
        ...
        if symmetric:
            ...
            u, crit_u[0], max_asgt = _mpbl_eval(
                ...
                criterion=criterion,
                objective=objective,
                n_edges_in=n_edges_in,
                n_edges_out=n_edges_out,
                asgt_u=asgt_u,
                crit_u=crit_u[0],
                max_asgt=max_asgt,
            )
    ...
    if symmetric:
        ...
        U_prop = (
            propagate_potentials(potentials_orig[0], max_asgt[0]),
