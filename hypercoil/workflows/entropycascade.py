...
def configure_model(n_labels, device='cuda:0', lr=0.05, saved_state=None):
    ...
    return atlas, opt, lh_coor, rh_coor, lh_mask, rh_mask
...
#TODO: replace with a torch scheduler that steps on epoch listen
def cut_lr(epoch, opt):
    if epoch == 500:
...
def entropy_cascade(epochs, out_root, ds, atlas, opt,
