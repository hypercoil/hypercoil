...
from hypercoil.engine import Epochs, LossArchive
...
from hypercoil.loss import ReducingLoss, QCFC
...
qcfc = QCFC(nu=1, tol=0.05, name='QC-FC')


qcfc_val = QCFC(nu=1, name='QC-FC_Val')


epochs = Epochs(max_epoch)
loss_tape = LossArchive(epochs=epochs)
qcfc.register_sentry(loss_tape)
...
def ingest_data(dl, wndw):
    ...
    confounds[torch.isnan(confounds)] = 0.001 * torch.randn_like(confounds[torch.isnan(confounds)])
    qc[torch.isnan(qc)] = 0.001 * torch.randn_like(qc[torch.isnan(qc)])

    [bold, confounds, qc, nanmask] = wndw((bold, confounds, qc, nanmask))
    return bold, confounds, qc, nanmask


def model_forward(model, bold, confounds, mask, qc, lossfn):
    bold, confmodel = model(bold, confounds, mask)
    cor = conditionalcorr(bold, confmodel)
    loss = lossfn(sym2vec(cor), qc.nanmean(1).unsqueeze(0))
    return loss, confmodel
...
for epoch in epochs:
    print(f'[ Epoch {epoch} ]')
    if epoch > 2:
        print(loss_tape.archive['QC-FC'][-3:], loss_tape.archive['QC-FC_Val'][-3:])
    dl_iter = iter(dl)
    selector.train()
    qcfc.tol = 0.1 * torch.rand(1).item()
    for _ in range(10):
        bold, confounds, qc, nanmask = ingest_data(dl_iter, wndw)
        loss, _ = model_forward(model, bold, confounds, nanmask.unsqueeze(1), qc, qcfc)
    ...
    with torch.no_grad():
        ...
        for _ in range(10):
            ...
            loss, _ = model_forward(model, bold, confounds, nanmask.unsqueeze(1), qc, qcfc_val)
